...
export default class MyBooked extends Component {
    constructor() {
        super()
        this.state = {
            allBooking: [],
    ...
    componentDidMount() {
        ...
        firebase.database().ref('bookings/').orderByChild('uid').equalTo(uid).on('value', (snap) => {
            if (snap.val()) {
                let pushkey = []
                let data = snap.val()
                for(let i in data){
            ...
            }else{this.setState({allBooking:[],pushKey:[]})}
        })
    }
    dltbooking(ind){
                        ...
                        return (
                            <div className="cardDiv" key={ind}>
                                <Card>
                                    ...
                                    <CardText>
                                        <b> Date : </b> {val.Date.slice(3, 15)}<br />
